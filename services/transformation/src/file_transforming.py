...
def id_blob_to_bq(blob_name):
    blob = bucket.blob(blob_name)
    id_json = blob.download_as_text()
    id_list = json.loads(id_json)
    flattened = []
    for item in id_list:
        idtf = item.get("document-id")
        flattened = extract_id(idtf)
...
def extract_id(id_dict):
    flattened_id = []
    flattened_id.append({"country": id_dict.get("country"),
                        "doc-number": id_dict.get("doc-number"),
                        "kind":id_dict.get("kind"),
                        "date": id_dict.get("date")})
    return flattened_id    

def class_ipcr_blob_to_bq(blob_name):
    blob = bucket.blob(blob_name)
    class_json = blob.download_as_text()
    class_list = json.loads(class_json)
    flattened = []
...
def class_uspc_blob_to_bq(blob_name):
    blob = bucket.blob(blob_name)
    uspc_json = blob.download_as_text()
    uspc_list = json.loads(uspc_json)
    flattened = []
    ...
    flattened = flattened_id | flattened_uspc
    df = pd.DataFrame(flattened)
    table_id = ""
    job = bq_client.load_table_from_dataframe(df, table_id)
    job.result()

def citation_blob_to_bq(blob_name):
    blob = bucket.blob(blob_name)
    citations_json = blob.download_as_text()
